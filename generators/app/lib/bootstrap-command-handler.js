...
export default class BootstrapCommandHandler {
    ...
    async _handleInteractiveSetup(options) {
        ...
        try {
            ...
            if (nonInteractive) {
                ...
                const ciAnswer = await this.generator.prompt([{
                    type: 'confirm',
                    name: 'useCi',
    ...
    async _handleUse(profileName) {
        if (!profileName) {
            ...
            console.log('       yo @aws/ml-container-creator bootstrap use none    (deactivate)')
            return
        }
    ...
    async _handleUpdate(options = {}) {
        ...
        if (!profile) {
            console.log('No active bootstrap profile found.')
            console.log('Run `yo @aws/ml-container-creator bootstrap` to set up shared infrastructure first.')
            return
        }
...
EXAMPLES:
  ...
  yo @aws/ml-container-creator bootstrap status --verify
  ...
  yo @aws/ml-container-creator bootstrap remove dev --force --delete-stack
  yo @aws/ml-container-creator bootstrap scan
  ...
  yo @aws/ml-container-creator bootstrap --non-interactive --profile my-aws-profile --region us-west-2 --ci
  yo @aws/ml-container-creator bootstrap --non-interactive --profile my-aws-profile --region us-west-2 --skip-ci
`)
    }
