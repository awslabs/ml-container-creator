...
// Generated: 2026-09-09T15:52:19.412Z
...
export const validationRules = {
    ...
    'backend': (value) => {
        if (!['http-flask','http-fastapi','transformers-vllm','transformers-sglang','transformers-tensorrt-llm','transformers-lmi','transformers-djl','triton-fil','triton-onnxruntime','triton-tensorflow','triton-pytorch','triton-vllm','triton-tensorrtllm','triton-python','diffusors-vllm-omni','marketplace'].includes(value)) return `Invalid value "${value}" for backend. Valid: http-flask, http-fastapi, transformers-vllm, transformers-sglang, transformers-tensorrt-llm, transformers-lmi, transformers-djl, triton-fil, triton-onnxruntime, triton-tensorflow, triton-pytorch, triton-vllm, triton-tensorrtllm, triton-python, diffusors-vllm-omni, marketplace`;
        return null;
    },
    'modelName': (value) => {
    ...
    'hpGpuCount': (value) => {
        if (value < 1) return `hpGpuCount must be >= 1, got ${value}`;
        if (value > 8) return `hpGpuCount must be <= 8, got ${value}`;
        return null;
    }
};

// 46 parameters have validation rules
