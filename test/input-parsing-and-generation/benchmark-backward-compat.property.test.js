    ...
    it('Property 3: do/deploy includes benchmark suggestion only when includeBenchmark is true', () => {
        ...
        fc.assert(fc.property(
            ...
            (baseConfig) => {
                ...
                const outputWith = ejs.render(deployTemplate, varsWithBenchmark, { filename: path.join(templatesDir, 'deploy.d/realtime-inference') });
                const outputWithout = ejs.render(deployTemplate, varsWithoutBenchmark, { filename: path.join(templatesDir, 'deploy.d/realtime-inference') });
