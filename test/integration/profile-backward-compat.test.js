...
describe('Profile Backward Compatibility (Tasks 7.1–7.4, PROF-4)', function () {
    ...
    describe('7.1 & 7.3: Old-format project — exports in do/config take precedence', function () {
        let result
        ...
        it('old ROLE_ARN export takes precedence over profile value', function () {
...
`)
        ...
        it('old ECR_REPOSITORY_NAME export takes precedence over profile value', function () {
            const output = runPrecedenceScript(`#!/bin/bash
...
`)
        ...
        it('shell environment AWS_REGION takes precedence over both config and profile', function () {
            const output = runPrecedenceScript(`#!/bin/bash
...
`, { AWS_REGION: 'eu-west-1' })
        ...
        it('do/config has the injected old-format exports (simulating v0.11.0)', function () {
        ...
        it('do/deploy still sources profile.sh (coexistence with old exports)', function () {
        ...
        it('do/deploy uses the precedence pattern for ROLE_ARN', function () {
            const deployContent = fs.readFileSync(result.file('do/deploy'), 'utf-8')
            assert.ok(
    ...
    describe('7.4: New-format project (v0.12.0) — no profile vars in do/config', function () {
        let result
        ...
        it('do/config does NOT contain export ECR_REPOSITORY_NAME', function () {
            const configContent = fs.readFileSync(result.file('do/config'), 'utf-8')
            assert.ok(
        ...
        it('do/config does NOT contain export ROLE_ARN', function () {
            const configContent = fs.readFileSync(result.file('do/config'), 'utf-8')
            assert.ok(
        ...
        it('do/config does NOT contain export ADAPTER_S3_BUCKET', function () {
            const configContent = fs.readFileSync(result.file('do/config'), 'utf-8')
            assert.ok(
        ...
        it('do/config does NOT contain export TUNE_S3_BUCKET', function () {
            const configContent = fs.readFileSync(result.file('do/config'), 'utf-8')
            assert.ok(
        ...
        it('do/config does NOT contain export CODEBUILD_PROJECT_NAME=', function () {
            const configContent = fs.readFileSync(result.file('do/config'), 'utf-8')
            assert.ok(
        ...
        it('do/config does NOT contain export ACCOUNT_ID', function () {
            const configContent = fs.readFileSync(result.file('do/config'), 'utf-8')
            assert.ok(
        ...
        it('do/config DOES retain export PROJECT_NAME', function () {
            const configContent = fs.readFileSync(result.file('do/config'), 'utf-8')
            assert.ok(
        ...
        it('do/config DOES retain export DEPLOYMENT_CONFIG', function () {
            const configContent = fs.readFileSync(result.file('do/config'), 'utf-8')
            assert.ok(
        ...
        it('do/lib/profile.sh exists in generated project', function () {
            result.assertFile('do/lib/profile.sh')
        })

        it('profile.sh has correct shebang', function () {
        ...
        it('profile.sh uses a single python3 call', function () {
            const content = fs.readFileSync(result.file('do/lib/profile.sh'), 'utf-8')
        ...
        it('do/deploy sources profile.sh', function () {
            const deployContent = fs.readFileSync(result.file('do/deploy'), 'utf-8')
            assert.ok(
                deployContent.includes('source "${SCRIPT_DIR}/lib/profile.sh"'),
        ...
        it('do/build sources profile.sh', function () {
        ...
        it('do/push sources profile.sh', function () {
        ...
        it('do/deploy uses _PROFILE[] for profile-resolved values', function () {
            const deployContent = fs.readFileSync(result.file('do/deploy'), 'utf-8')
            assert.ok(
        ...
        it('do/config contains profile-resolved comment indicating migration', function () {
            const configContent = fs.readFileSync(result.file('do/config'), 'utf-8')
            assert.ok(
    ...
    describe('7.5: New-format do/deploy reads from profile via _PROFILE[] (PROF-4)', function () {
        let result
        ...
        it('do/deploy references _PROFILE[roleArn] for ROLE_ARN resolution', function () {
            const deployContent = fs.readFileSync(result.file('do/deploy'), 'utf-8')
            assert.ok(
                deployContent.includes('_PROFILE[roleArn]'),
        ...
        it('do/deploy references _PROFILE[ecrRepositoryName] for ECR resolution', function () {
            const deployContent = fs.readFileSync(result.file('do/deploy'), 'utf-8')
            assert.ok(
                deployContent.includes('_PROFILE[ecrRepositoryName]'),
        ...
        it('do/deploy references _PROFILE[awsRegion] for AWS_REGION resolution', function () {
            const deployContent = fs.readFileSync(result.file('do/deploy'), 'utf-8')
            assert.ok(
        ...
        it('profile values are used when env vars are not set (precedence model)', function () {
...
`)
        ...
        it('profile ECR repository name is used when env var is not set', function () {
            const output = runPrecedenceScript(`#!/bin/bash
...
`)
    ...
    describe('7.6: do/deploy without bootstrap profile — clear error message (PROF-4)', function () {
        let result
        ...
        it('do/deploy has validation check for empty ROLE_ARN', function () {
            const deployContent = fs.readFileSync(result.file('do/deploy'), 'utf-8')
            assert.ok(
        ...
        it('do/deploy error message mentions bootstrap command', function () {
            const deployContent = fs.readFileSync(result.file('do/deploy'), 'utf-8')
            assert.ok(
        ...
        it('do/deploy error message mentions setting ROLE_ARN as env var', function () {
            const deployContent = fs.readFileSync(result.file('do/deploy'), 'utf-8')
            assert.ok(
        ...
        it('when both profile and env are empty, ROLE_ARN is empty (triggers error)', function () {
...
`)
        ...
        it('script exits with error code when ROLE_ARN is empty', function () {
...
`)
    ...
    describe('7.7: AWS_REGION=us-east-1 do/deploy — shell env wins (PROF-4, PROF-5)', function () {
        it('shell AWS_REGION overrides profile awsRegion', function () {
            const output = runPrecedenceScript(`#!/bin/bash
...
`, { AWS_REGION: 'us-east-1' })
        ...
        it('shell ROLE_ARN overrides profile roleArn', function () {
            const output = runPrecedenceScript(`#!/bin/bash
...
`, { ROLE_ARN: 'arn:aws:iam::555666777888:role/shell-role' })
        ...
        it('shell ECR_REPOSITORY_NAME overrides profile ecrRepositoryName', function () {
            const output = runPrecedenceScript(`#!/bin/bash
...
`, { ECR_REPOSITORY_NAME: 'shell-override-repo' })
        ...
        it('AWS_REGION is exported (visible to child processes)', function () {
            const output = runPrecedenceScript(`#!/bin/bash
export AWS_REGION=\${AWS_REGION:-us-west-2}
_PROFILE_awsRegion="ap-southeast-1"
export AWS_REGION="\${AWS_REGION:-\${_PROFILE_awsRegion:-us-east-1}}"
...
`, { AWS_REGION: 'us-east-1' })
        ...
        it('do/deploy template uses export for AWS_REGION', function () {
    ...
    describe('Precedence model: env var > profile > default (Tasks 7.1-7.3)', function () {
        it('when env var is set, profile value is ignored', function () {
            const output = runPrecedenceScript(`#!/bin/bash
...
`)
        ...
        it('when env var is empty, profile value is used', function () {
            const output = runPrecedenceScript(`#!/bin/bash
# Env var not set
unset ROLE_ARN
_PROFILE_roleArn="profile-value"
...
`)
        ...
        it('when both env var and profile are empty, fallback default is used', function () {
            const output = runPrecedenceScript(`#!/bin/bash
unset ECR_REPOSITORY_NAME
...
`)
        ...
        it('AWS_REGION three-level precedence: shell > config > profile > default', function () {
...
`)
        ...
        it('values containing special characters (ARNs, S3 paths) preserve correctly', function () {
            const output = runPrecedenceScript(`#!/bin/bash
